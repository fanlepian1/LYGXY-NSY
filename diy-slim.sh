...
# 避免 patch-kernel.sh 删除 rust vendor 的 Cargo.toml.orig
# https://github.com/openwrt/packages/issues/27485
if grep -qF "*.orig" scripts/patch-kernel.sh; then
	sed -i "s/-name '\\*\\.orig' -o //g" scripts/patch-kernel.sh
fi
